...
Plots.scatter(xrange[xsort],y_reg[xsort],markersize=2.0)
Plots.plot!(sort(xrange),Optim.minimizer(xmap)[xsort],linewidth=3.0,color=:blue)
Plots.fill_between!(xpred,μgp .- 2*sqrt.(max.(0.0,siggp)),μgp .+ 2*sqrt.(max.(0.0,siggp)),where=trues(length(xpred)),color=:red,alpha=0.2)
Plots.plot!(xpred,μgp,ribbon=2*sqrt.(max.(0.0,siggp)),linewidth=3.0,color=:red,fillalpha=0.2)
...
X_t = []
η=0.01
opt_x=[Descent(η),Descent(η)]
move_particles(x_t,p_stu,opt_x)
μp,sigp = m_and_C(x_t)
sigp = diag(sigp)
Plots.plot!(xrange[xsort],μp[xsort],ribbon=2*sqrt.(sigp),linewidth=3.0,color=:green)
##
μp,sigp = m_and_diagC(x_t)
μf, σf = predic_f(p_reg,x_t,reshape(xpred,:,1))
Plots.plot(xpred,μf,color=:green,linewidth=2.0)
Plots.fill_between(xpred,μf.-sqrt.(max.(0.0,σf)),μf.+sqrt.(max.(0.0,σf)),color=:green,where=trues(length(xpred)))
...
Plots.scatter(vec(X),y_log,lab="data")
Plots.plot!(xrange[xsort],Optim.minimizer(xmap)[xsort],lab="MAP",lw=3.0)
Plots.plot!(xrange[xsort],μgp,ribbon=2*sqrt.(siggp),lab="GP",lw=3.0,fillalpha=0.2)
## Training with Classification
M = 200

# x_init = rand(MvNormal(x_0.minimizer,0.01),M)
x_init = rand(MvNormal(zero(y_log),0.01),M)
x_t = copy(x_init)
X_t = []
η=0.01
opt_x=[Descent(η),Descent(η)]
for i in 1:10
    move_particles(x_t,p_log,opt_x,cb=cb)
end
