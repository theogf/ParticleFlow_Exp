...
"""
struct SVGD{AD,F,K} <: VariationalInference{AD}
    device::F
    kernel::K
    max_iters::Int        # maximum number of gradient steps used in optimization
end

SVGD(device::F, kernel::K, args...) where {F,K} = SVGD{ADBackend(),F,K}(device, kernel, args...)
SVGD() = SVGD(identity, SqExponentialKernel(), max_iters)
...
function optimize!(
    ...
    K = alg.device(zeros(q.dist.n_particles, q.dist.n_particles))
    diff_mu = Inf
    time_elapsed = @elapsed while (i < max_iters) # & converged
        ...
        if alg.kernel isa TransformedKernel || alg.kernel isa LinearKernel
            for k = 1:q.dist.n_particles
                Δ[:, k] .= mean(
                    ...
                    # mean(
                    #     -K[j, k] * neg_gradlogp[:, j] + Zygote.gradient(
                    #         Base.Fix2(alg.kernel, q.dist.x[:, k]),
                    #         q.dist.x[:, j],
                    #     )[1] for j in 1:q.dist.n_particles
                    # )
        ...
        q.dist.x .+= apply!(optimizer, q.dist.x, Δ)
        if any(isnan, q.dist.x)
            @warn "NaN values appeared, stopping simulation"
            break
        end
