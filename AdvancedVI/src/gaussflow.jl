...
Can only work on the following distributions:
 ...
 - `BlockMFMvNormal`
"""
struct GaussFlow{AD,RNG,F} <: GVA{AD}
    rng::RNG
    device::F
...
GaussFlow(rng, device, args...) = GaussFlow{ADBackend(),typeof(rng),typeof(device)}(rng, device, args...)
GaussFlow() = GaussFlow(Random.GLOBAL_RNG, identity, 100, 10, false, false)
...
function optimize!(
    ...
    x = alg.device(zeros(length(q.dist), samples_per_step)) # Storage for samples
    ...
    time_elapsed = @elapsed while (i < max_iters) # & converged
        ...
        Distributions.rand!(alg.rng, q, x) # Sample from q
