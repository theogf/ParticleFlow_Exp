...
function run_gp_gpf(exp_p)
    ...
    for i = 1:n_runs
        ...
        x_init = rand(MvNormal(Optim.minimizer(opt_MAP), σ_init), n_particles) # Initialize particle around the MAP
        h = MVHistory()
        general_p =
        ...
        vi, q = init_gflow(gflow_p, general_p) # Initialization of the objects
        ...
        tagsave(
        datadir("results", "gp", dataset, @savename(n_particles), file_prefix * ".bson"),
