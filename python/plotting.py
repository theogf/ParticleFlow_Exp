...
from line_search import find_optimal_learning_step_sizes_c, line_search_on_learning_rate_c, \
    optimize_with_optimal_rate_c
...
def simple_gaussian_initial_rate_scatter():
    ...
    for dim in [2, 4, 8, 16, 32, 64]:
        for seed in range(8):
            x = find_optimal_learning_step_sizes_c(seed, max_particle_exp=2, dim=dim)
            data.extend(x)
...
def simple_gaussian_increased_particles(regenerate_data=True):
    ...
    if regenerate_data:
        ...
        for dim in [2, 4, 8, 16, 32, 64]:
            for seed in range(8):
                x = find_optimal_learning_step_sizes_c(seed, max_particle_exp=10, dim=dim)
                data.extend(x)
        df = pd.DataFrame(data)
...
def simple_gaussian_over_time_momentum(initial_particles_mult=1.0, num_particles_mult=1.0):
    ...
    for seed in range(8):
        ...
        lr, _ = line_search_on_learning_rate_c(gaussian, particles)
